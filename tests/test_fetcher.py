...
from digest_pipeline.fetcher import Paper, _download_pdf, _within_last_24h, fetch_papers
...
def test_download_pdf_success(mock_get, tmp_path):
    ...
    assert _download_pdf("http://example.com/paper.pdf", dest, max_retries=3) is True
    assert dest.read_bytes() == b"%PDF-fake-content"


@patch("digest_pipeline.fetcher.requests.get", side_effect=Exception("network error"))
def test_download_pdf_all_retries_fail(mock_get, tmp_path):
    dest = tmp_path / "test.pdf"
    assert _download_pdf("http://example.com/paper.pdf", dest, max_retries=2) is False
